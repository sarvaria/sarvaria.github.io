...
/**
 * Sets the standard HP header and footer based on the requested locale and the
 * HP internet design standards.
 */

Util.setHPCAAS = function() {
    ...
    // Searching the locale parameter in the URL
    
    if (window.location.search) {
        searchLang = Util.getURLParameters().locale || browserLanguage;
    } else {
        searchLang = browserLanguage;
    }

    // Is the requested locale supported?
    
    var p = sl.indexOf(searchLang.replaceAll('/', ''));
    
    if (p >= 0) {
        lang = sl[p];
    }

    // Getting the CAAS country and language
    ...
    // Loading the CAAS libraries

    var s = document.createElement('script');
    s.type = 'text/javascript';
    s.setAttribute('src', 'https://ssl.www8.hp.com/' + l + '/scripts/framework/jquery/v-1-8/can.jquery.js');
    ...
    var timer = setInterval(function() {
        if (can) {
            ...
            s.type = 'text/javascript';
            s.setAttribute('src', 'https://ssl.www8.hp.com/caas/header-footer/' + l + '/default/latest?contentType=js&hide_country_selector=false,&hide_mobile_link=true');
            document.body.appendChild(s);
        }
    }, 100);
};